...
远程开发入口：双击 `develop-on-sen3.cmd` 在 sen3 上使用 Codex；完成提交和推送后，双击 `update-from-sen3.cmd` 更新 Windows。详见 [服务器开发说明](docs/服务器开发.md)。
...
- `T2`：将所选文字刷为 `HZ` 样式、宽度比例 `0.7`，高度 `3` 若与周围对象间距不足 `0.3` 则自动降低到可用的最大值，并保持文字中心位置。
- `QSTXT`：从选择内容中保留文字对象。
- `QW`：修改文字高度。
